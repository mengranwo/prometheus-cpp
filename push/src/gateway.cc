...
namespace prometheus {
    ...
    void Gateway::RegisterCollectable(const std::weak_ptr<Collectable> &collectable,
        ...
        collectables_.emplace_back(std::make_pair(collectable, ss.str()));
    }

    int Gateway::push(PushMode mode) {
        const auto serializer = WavefrontSerializer{};
        // iterate through all the registries
        for (auto &entry : collectables_) {
            auto register_pr = entry.first.lock();
            if (!register_pr) {
                continue;
            }

            auto metrics_family = std::vector<MetricFamily>{};
            for (auto metrics : register_pr->Collect()) {
                metrics_family.push_back(metrics);
            }
            // construct target uri to establish connection
            std::stringstream uri;
            uri << jobUri_ << labels_ << entry.second;
            // serialize all the metrics(counter/gauge/histogram)
            auto body = serializer.Serialize(metrics_family);
